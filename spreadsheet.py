...
def vtingsheet(articleid,vernum):
...
#Find the row/rows in the spreadsheet that correspond to the given articleid 
 ...
 row_vermatch=[i for i, e in enumerate(ingsheet_version) if e == vernum]
...
#Get the Requestion, Version, IngestDate, Title, Comment, ArticleID, IngestNumber that correspond the rownumber found above   
 ...
 dictingsheet=dict({'ingrownum': rownum+1,'ingestno':ingest_no,'ingrequestr':ing_requestor,'ingversion':ing_version,'ingestdate':ing_date,'ingtitle':ingsheet_title,'ingcomment':ing_comment,'ingarticleid': ing_articleid})
 return dictingsheet


#Following gets information from the spreadsheet "Published" sheet using the article id and version
